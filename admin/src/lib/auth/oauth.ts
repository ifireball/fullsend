...
/**
 * Vite injects `import.meta.env.BASE` from `vite.config.ts` `base`. If it is missing
...
export function getGithubAppClientId(): string {
  ...
  if (!id) {
    throw new Error(
      "Missing GITHUB_APP_CLIENT_ID — add it to admin/.env.local (see sample.env.local)",
    );
  }
  return id;
}
