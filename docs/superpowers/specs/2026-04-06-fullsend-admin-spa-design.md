...
| Organization memberships (admin org list) | `GET https://api.github.com/user/memberships/orgs` (browser / Octokit `paginate`) | Bearer user token; SPA calls GitHub directly (CORS allows this route). **Classic OAuth scopes:** `read:org` is required to see **private** organization memberships; without it GitHub may omit private orgs (see GitHub REST docs for this endpoint). **GitHub App user-to-server tokens** follow the **App installation permissions** and org access rules instead of `X-OAuth-Scopes`; preflight scope lists in `internal/layers/*` do not yet include org-listing (derive from GitHub when tightening install defaults). |
