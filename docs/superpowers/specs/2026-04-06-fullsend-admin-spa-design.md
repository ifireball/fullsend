...
| OAuth authorize start (no client id in static bundle) | `GET /api/oauth/authorize` → `302` to `GET https://github.com/login/oauth/authorize` | Query: `redirect_uri`, `state`, `code_challenge`, `code_challenge_method=S256`. Site Worker validates `redirect_uri` and tab origin, then redirects to GitHub with `client_id` from Worker env only. |
