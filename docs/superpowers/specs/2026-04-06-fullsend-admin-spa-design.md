...
| Deploy readiness — classic OAuth scope gate (org list **Deploy Fullsend**) | Same as optional preflight: `HEAD https://api.github.com/user` → **`X-OAuth-Scopes`** | Compared to the deduped install set **`repo`**, **`workflow`**, **`admin:org`** (mirrors Go `Stack.CollectRequiredScopes(OpInstall)` across `ConfigRepoLayer`, `WorkflowsLayer`, `SecretsLayer`, `EnrollmentLayer`, `DispatchTokenLayer`). When the header lists granted scopes and any of these are **missing**, the org list shows **Cannot deploy** with per-scope lines. When comparison is **skipped** (no header), **Deploy** is not blocked on OAuth-scope grounds. **Org role / policy** (e.g. permission to create repositories) is not inferred here. |
| Admin GitHub App — repository permissions (install + analyze parity) | GitHub App **Settings → Repository permissions** | **Metadata:** Read. **Contents:** Read and write. **Actions:** Read and write (repo secrets/variables and org-style checks on `.fullsend`). **Workflows:** Read and write (managed workflow files). **Administration:** may be required to **create** an org-owned repository via API depending on organisation policy — confirm when the install wizard is implemented. |
| Admin GitHub App — organisation permissions | GitHub App **Settings → Organisation permissions** | **Secrets** (org-level Actions secrets): Read and write — required for dispatch-token layer org secret read/update and matches classic OAuth **`admin:org`** for PATs. Third-party app access, SAML SSO app authorization, and repository selection for the installation still apply. |
| Org list — can user create an org-owned repo? | `GET https://api.github.com/user/memberships/orgs` (paginated) | Each membership may include `permissions.can_create_repository` (GitHub `org-membership` schema). There is **no** dry-run for `POST /orgs/{org}/repos`; this flag is the supported read-only signal. `null` in the SPA means the user was not listed as an org member (e.g. outside collaborator only) or the membership fetch failed — the UI then falls back to repository `permissions` from `GET /user/repos`. |
